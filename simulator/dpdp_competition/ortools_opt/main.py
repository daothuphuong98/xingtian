...
class OrtoolsOptimizer():
    def __init__(self) -> None:
        self.config = Configs()
        self.ortools_config = OrtoolsConfigs()
    ...
    def solution_to_json(self, data, manager, routing, solution):
        ...
        with open(self.config.algorithm_output_destination_path, 'w') as f:
            json.dump(destination_output, f, indent=4)
        with open(self.config.algorithm_output_planned_route_path, 'w') as f:
    ...
    def create_routing_model(self):
        ...
        routing.AddDimension(
            ...
            0,  # no slack
            100_000_000_000,  # vehicle maximum travel time
            True,  # start cumul to zero
            time)
        time_dimension = routing.GetDimensionOrDie(time)
        ...
        for location_idx, time_window, demand in zip(range(len(data['demands'])), data['time_windows'], data['demands']):
            if time_window > 0:
                ...
                if demand < 0:
                    time_dimension.SetCumulVarSoftUpperBound(index, time_window, self.ortools_config.TIMEOUT_PENALTY)
        ...
        for node in data['droppable']:
            routing.AddDisjunction(
                ...
                self.ortools_config.DROPPABLE_PENALTY) 
        ...
        return routing, manager, data

    def run(self):
        routing, manager, data = self.create_routing_model()
        ...
        if previous_solution:
            ...
            self.print_solution(data, manager, routing, previous_solution)
            solution = routing.SolveFromAssignmentWithParameters(previous_solution, search_parameters)
            print("Solver LCI status: ", routing.status())
            self.print_solution(data, manager, routing, solution)
            self.solution_to_json(data, manager, routing, solution)
        else:
            routing2, manager, data = self.create_routing_model()
            search_parameters.first_solution_strategy = (
                routing_enums_pb2.FirstSolutionStrategy.PATH_CHEAPEST_ARC)
            solution = routing2.SolveWithParameters(search_parameters)
            print("Solver PCA status: ", routing2.status())
            self.print_solution(data, manager, routing2, solution)
            self.solution_to_json(data, manager, routing2, solution)
